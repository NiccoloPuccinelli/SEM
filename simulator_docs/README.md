...
The current repository constitutes the backend logic of a ride-sharing simulator written in Python. The logic interacts with [SUMO](https://sumo.dlr.de/docs/index.html) (a popular traffic generator and simulator) through [TraCI](https://sumo.dlr.de/docs/TraCI/Interfacing_TraCI_from_Python.html) an API interface specifically developed for Python (you can find the documentation [here](https://sumo.dlr.de/pydoc/traci.html)).

## 2. Documentation

The documentation of the simulator (the architecture, the life-cycle, the logic, and the main functionalities) are described [here](Documentation.md).
...
For **Linux users**, in case you are unable to install SUMO version 1.19.0, please refer to the following [guide](https://sumo.dlr.de/docs/Installing/Linux_Build.html).

### 3.2 Python 3

The logic of the ride-sharing simulator is written in **Python 3**. Therefore, a stable version of Python 3 (for example, 3.6 or more) is necessary to run the simulator. Making several tests, we recommend **3.6 <= Python version <= 3.10**.
...
Finally, the `scenario` sub-folder contains at least one scenario that the simulator can simulate (the simulator can run one scenario at a time, set the `SCENARIO` environment variable). The scenario is provided through a configuration file, with the following structure:
...
In the demos made available with the repository, 6 scenario has been defined:
...
* `drivers strike`: The simulator reproduces a scenario in which the drivers progressively stop to work putting in place a strike.
* `long rides`: The simulator reproduces a scenario in which the customers progressively increase the length of the ride requests.
* `underground failure`: The simulator reproduces a scenario in which an underground failure occurs in some TAZs of the city. The underground failure generates a big number of requests in few hundreds timestamps, within the TAZs involved in the failure.
* `flash mob`: The simulator reproduces a scenario in which a flash mob is performed in some TAZs of the city. The flash mob is emulated decreasing drastically the speed of the street in the TAZ where it takes place (followed by a speed up of the same streets when the flash mob ends).
...
The simulator is developed to run simulations on arbitrary (real or fantasy) urban nets. To run the simulator it is necessary to initialize the simulator:
...
First, set the SUMO_HOME variable so that it is visible to the simulator. To do this, follow these steps in the terminal:
...
The `.env` file must contain the following environment variables:
    ...
    DEFAULT_ROUTES      = [ If set to True (Y), the simulator computes a route for each edge in each TAZ, with each other edge in the TAZ as destination ]
...
The `DRIVER_SUPPORT` generates additional drivers at the beginning of the simulation, to decrease the probability to have customers requests at the beginning of the simulation, without drivers active in the net yet. As an example, if the `DRIVER_SUPPORT` is equal to `n` the simulator will generate `n` drivers (from `0` to `10`) in the first timestamp of the simulation, for each TAZ of the net, if the pickup value in the begin hour of each specific TAZ is greater than 30.

The `POLY_CONVERSION` is a boolean environment variable. Its meaning is explained in the previous [section](#the-data-folder). It must be set to `True` if the layout of the net layers are provided in a shape file. The poly-conversion transforms the shape file in an XML file that can be read by SUMO.

The `TEST_NET` is another boolean environment variable explained in the previous [section](#the-data-folder). It must be set to `True` if the mobility of the net is not provided by the developer. In this case, the simulator will generate a random mobility for the simulation.

The `ADVANCE_DISTANCE` boolean environment variable computes the driving time and distance between each couples of edges (roads) in the net, if set to `TRUE`. This process will speed up the computation of the routes at simulation time (since the calculation will be performed before the simulation), but it will increase the time to process the net and configure the simulation. Moreover this is an expensive memory operation. It is recommended to set this variable `True` only if the net is small (like the `city` demo net).
...
A concrete example of `.env` file is the following, to test the small fake city net provided within the repository:

    CITY=city
    LAYER_BOUNDARY=test
    LAYER_MOBILITY=test
    LAYER_ANALYTICS=test
    NET_SUMO=city.net
...
Pay attention to the answers for `init_env.py`. In the current version of the simulator Y/N are required instead of True/False.
...
To save the net for future use, without recompute it, after other nets will be used.
...
Otherwise, the command will open a SUMO Graphical User Interface (SUMO GUI) and will upload the simulation net (we use the `city` demo net as example, in the following instructions).
